...
def get_dataloader(data_dir: Path, target_sr: int, batch_size: int = 8, max_samples: int = 48000):
    ...
    print(f"Loading LibriTTSR dataset from {data_dir}...")
    base_dataset = LibriTTSR(data_dir=str(data_dir))
    
    train_dataset = TrainDatasetWrapper(base_dataset, split="train")
    
    def collate_fn(batch):
        wavs = []
        for item in batch:
            wav = item["audio_input"][0]
            if wav.dim() > 1:
                wav = wav.mean(dim=0)
            if wav.shape[0] > max_samples:
                wav = wav[:max_samples]
            # audio is already normalized by TrainDatasetWrapper
            wavs.append(wav)
        return wavs
        
    return torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn, num_workers=4)
