...
If ``--target-audio`` is provided, swap behavior is controlled by ``--swap-part`` and ``--swap-alpha``:
- quantized (default): swap only quantized embeddings on VQ head; ignore head residual
- residual: swap only VQ-head residual from target; keep source quantized head (and tail per ``--mode``)
- residual_and_tail: swap VQ-head residual and non-quantized tail from target; keep source quantized head
- tail: swap only non-quantized tail; keep source VQ head
- ``--swap-alpha`` in ``[0, 1]``: 1 = full target on swapped slices, 0 = no swap (source only; target not encoded)

Post-processing (after swap, before decode): ``--tail-smooth none|mean|ma`` applies the same
temporal smoothing to the **non-quantized tail** and to the **VQ-head residual** ``r`` (global mean
or centered moving average with ``--tail-ma-window``, odd). Smoothing uses only **valid** latent
frames (encoder ``padding_mask``); padded suffix is left unchanged. ``--mean`` is a shorthand for
``mean``.

Typical recipe — decode ``both`` (``q+r``) with target residual+tail blend and MA:

``python eval_vq_components.py ... --mode both --swap-part residual_and_tail --target-audio ... --tail-smooth ma --tail-ma-window 7``

Inference keeps dropout off: ``model.eval()``, ``vocoder.eval()``, and ``torch.inference_mode()`` on encode/decode.
"""
...
def _odd_ma_window(k: int) -> int:
    if k < 1:
        raise ValueError("tail_ma_window must be >= 1")
    if k % 2 == 0:
        return k + 1
    return k


def _valid_latent_length(pad_mask: Optional[torch.Tensor], T: int, b: int) -> int:
    """Encoder convention: padding_mask True = padded; valid is prefix of length L."""
    if pad_mask is None:
        return T
    return int((~pad_mask[b]).sum().item())


def smooth_latent_temporal(
    x_bt_d: torch.Tensor,
    mode: str,
    ma_window: int,
    latent_pad_mask: Optional[torch.BoolTensor] = None,
) -> torch.Tensor:
    """
    Smooth latents [B, T, D] along time (tail and/or VQ-head residual).

    ``latent_pad_mask`` [B, T]: True = padded (ignored for mean/MA; values copied through).

    mode: "none" -> unchanged; "mean" -> mean over valid frames only, written on valid frames;
    "ma" -> centered moving average with reflect padding on the valid prefix only.
    """
    if mode == "none":
        return x_bt_d
    B, T, D = x_bt_d.shape
    device = x_bt_d.device
    out = x_bt_d.clone()

    if mode == "mean":
        for b in range(B):
            vl = _valid_latent_length(latent_pad_mask, T, b)
            if vl <= 0:
                continue
            m = x_bt_d[b, :vl, :].mean(dim=0, keepdim=True)  # [1, D]
            out[b, :vl, :] = m.expand(vl, -1)
        return out

    if mode == "ma":
        k = _odd_ma_window(int(ma_window))
        if k == 1:
            return x_bt_d
        pad = k // 2
        w = torch.full((1, 1, k), 1.0 / k, device=device, dtype=torch.float32)
        for b in range(B):
            vl = _valid_latent_length(latent_pad_mask, T, b)
            if vl <= 0:
                continue
            seg = x_bt_d[b : b + 1, :vl, :]  # [1, vl, D]
            x = seg.reshape(D, 1, vl).float()
            x = F.pad(x, (pad, pad), mode="reflect")
            y = F.conv1d(x, w.to(dtype=x.dtype), padding=0)
            out[b, :vl, :] = y.reshape(1, vl, D).to(dtype=x_bt_d.dtype)
        return out

    raise ValueError(f"Unknown tail smooth mode: {mode}")
...
def encode_vq_components(
    ...
    torch.Tensor, torch.Tensor, torch.Tensor, torch.BoolTensor, Optional[torch.Tensor]
]:
    """
    Returns:
...
def run(
    *,
    ...
    tail_smooth: str = "none",
    tail_ma_window: int = 7,
    ...
    swap_alpha = float(min(max(swap_alpha, 0.0), 1.0))
    if target_audio_tensor is not None and swap_alpha > 0.0:
        q_tgt, r_tgt, tail_tgt, _, _ = encode_vq_components(
            model, target_audio_tensor, target_sr
        )
        t_use = min(q.shape[1], q_tgt.shape[1])
        if q.shape[1] != q_tgt.shape[1]:
            logger.warning(
                "Latent time mismatch source=%s target=%s; applying %s swap on min length=%s.",
                q.shape[1],
                q_tgt.shape[1],
                swap_part,
                t_use,
        ...
        if swap_part == "quantized":
            q = q_src.clone()
            q[:, :t_use, :] = torch.lerp(q_src[:, :t_use, :], q_tgt[:, :t_use, :], a)
        elif swap_part == "residual":
            r = r_src.clone()
            r[:, :t_use, :] = torch.lerp(r_src[:, :t_use, :], r_tgt[:, :t_use, :], a)
        elif swap_part == "residual_and_tail":
            r = r_src.clone()
            r[:, :t_use, :] = torch.lerp(r_src[:, :t_use, :], r_tgt[:, :t_use, :], a)
            tail = tail_src.clone()
            tail[:, :t_use, :] = torch.lerp(
                tail_src[:, :t_use, :], tail_tgt[:, :t_use, :], a
            )
        elif swap_part == "tail":
            tail = tail_src.clone()
            tail[:, :t_use, :] = torch.lerp(
    ...
    if tail_smooth != "none":
        tail = smooth_latent_temporal(
            tail, tail_smooth, tail_ma_window, latent_pad_mask=pad_src
        )
        r = smooth_latent_temporal(
            r, tail_smooth, tail_ma_window, latent_pad_mask=pad_src
        )

    # In swap mode, ignore VQ residual on the head entirely:
    # use only swapped quantized embeddings for the VQ head.
    if (
        target_audio_tensor is not None
        and swap_alpha > 0.0
        and swap_part == "quantized"
    ):
        head = q
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="quantized",
        choices=["quantized", "residual", "residual_and_tail", "tail"],
        help=(
            "Which latent part to swap from target: quantized VQ head, VQ-head residual, "
            "residual+tail together, or non-quantized tail only."
        ),
    )
    p.add_argument(
        "--swap-alpha",
        type=float,
        default=1.0,
        help="Swap strength in [0, 1] when --target-audio is set: 1 full target blend, 0 source only.",
    )
    p.add_argument(
        "--tail-smooth",
        type=str,
        default="none",
        choices=["none", "mean", "ma"],
        help="Smooth tail and VQ-head residual r over time: none, global mean, or MA (see --tail-ma-window).",
    )
    p.add_argument(
        "--tail-ma-window",
        type=int,
        default=7,
        help="Moving-average kernel length (odd; if even, bumped to k+1). Used when --tail-smooth ma.",
    )
    p.add_argument(
        "--mean",
        action="store_true",
        help="Shorthand for --tail-smooth mean (mutually exclusive with --tail-smooth other than none).",
    )
...
def main() -> None:
    ...
    if not 0.0 <= args.swap_alpha <= 1.0:
        raise ValueError("--swap-alpha must be in [0, 1].")
    if args.mean:
        if args.tail_smooth not in ("none", "mean"):
            raise ValueError(
                "--mean cannot be combined with --tail-smooth=%s" % args.tail_smooth
            )
        tail_smooth = "mean"
    else:
        tail_smooth = args.tail_smooth
    if tail_smooth == "ma":
        if args.tail_ma_window < 1:
            raise ValueError("--tail-ma-window must be >= 1")
        if args.tail_ma_window % 2 == 0:
            logger.info(
                "Using odd --tail-ma-window %s (was %s)",
                _odd_ma_window(args.tail_ma_window),
                args.tail_ma_window,
            )

    device = torch.device(args.device) if args.device else default_inference_device()
    ...
    if args.target_audio is not None:
        wav_tgt = load_wav_mono_resampled(
            args.target_audio, ENCODER_INPUT_SR_HZ, device
        )
    ...
    _, mel_valid = run(
        ...
        tail_ma_window=args.tail_ma_window,
    )
    ...
    alpha_tag = f"_a{args.swap_alpha:g}" if args.target_audio is not None else ""
    if tail_smooth == "mean":
        smooth_tag = "_meanTail"
    elif tail_smooth == "ma":
        smooth_tag = f"_ma{_odd_ma_window(args.tail_ma_window)}Tail"
    else:
        smooth_tag = ""
    out_name = (
        f"{stem}{swap_tag}{alpha_tag}{smooth_tag}_{args.mode}_"
