...
Usage:
  python evaluation/run_tail_sweep.py --checkpoint checkpoints/paper/setting30/model.safetensors --chunks 1 2 3 4 --chunk-size 4 --upload-wandb --save-audio
"""
...
def make_plot(results, out_path: Path, title: str):
    ...
    ax1.plot(chunks, wers,  marker="o", linewidth=2, color="#1f77b4", label="WER (reconstructed)")
    ax1.plot(chunks, cers,  marker="s", linewidth=2, color="#ff7f0e", label="CER (reconstructed)")
    ...
    # UTMOS on right axis
    ax2.plot(chunks, utmos, marker="^", linewidth=2, color="#2ca02c", label="UTMOS (reconstructed)")
...
def print_report(results, chunk_size):
    print("\n" + "=" * 70)
    print(f"{'Chunks (N)':>12} {'chunk_size':>12} {'WER (%)':>10} {'CER (%)':>10} {'UTMOS':>10}")
    print("-" * 70)
    print(f"{'reference':>12} {'—':>12} {REF_WER:>10.2f} {REF_CER:>10.2f} {REF_UTMOS:>10.2f}")
    print("-" * 70)
    for r in results:
        print(f"{r['chunk']:>12} {chunk_size:>12} {r['WER']:>10.2f} {r['CER']:>10.2f} {r['UTMOS']:>10.2f}")
    print("=" * 70 + "\n")
...
def main():
    ...
    parser.add_argument("--chunks", type=int, nargs="+", default=[1, 2, 4, 8, 16], help="Chunk counts to evaluate")
        ...
        if run is not None:
            ...
            if args.save_audio:
                ...
                for i in range(min(3, len(samples))):
                    ...
                    torchaudio.save(str(wav_path), recon_audio.unsqueeze(0), TARGET_SR)
                    log_dict[f"audio/chunk_{chunk}_sample_{i}"] = wandb.Audio(str(wav_path), caption=f"Chunk {chunk} - {s['id']}", sample_rate=TARGET_SR)
    ...
    make_plot(all_results, plot_path, title=f"Metrics vs latent bottleneck — {setting_name} ({args.num_samples} samples)")
