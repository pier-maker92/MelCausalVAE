...
def search_path(z_p, m_p, logs_p, x_mask, y_mask):
    with torch.no_grad():
        ...
        # z_p: [B, C, T_audio] (Variabili latenti audio trasformate dal flow)
...
class VitsKLProcessor(nn.Module):
    ...
    def forward(self, z_spec, y_mask, phonemes, phoneme_lengths):
        ...
        Nota: questa classe calcola la NLL di z_flow sotto il prior del testo,
        meno il log-determinante del flow. Se z_spec è detachato, i gradienti
        fluiscono solo al text encoder e al flow (l'aligner "insegue" il decoder).
        """
        ...
        # Normalizzazione per numero totale di frame nel batch
...
class Aligner(nn.Module):
    ...
    def forward(
        ...
        Nota: se z_spec è detachato dal grafo computazionale, i gradienti della loss
        fluiscono solo ai componenti interni dell'aligner (text encoder + flow),
        facendo sì che l'aligner "insegua" il decoder senza influenzare l'encoder.
        """
        ...
        # 5. Calcolo Loss: NLL(z_flow; prior) - logdet
        # Questa è la negative log-likelihood di z_flow sotto il prior del testo,
        # meno il log-determinante del flow. NON è la KL completa (manca l'entropia
        # del posterior), il che è intenzionale quando z_spec è detachato:
        # l'aligner deve solo imparare ad allineare, non regolare il posterior.
        kl_loss_elements = logs_p_expanded + 0.5 * torch.exp(-2 * logs_p_expanded) * (
            (z_flow - m_p_expanded) ** 2
        )
        ...
        # 6. Pooling: media pesata dei frame audio per ciascun fonema
        # Normalizzazione per durata di ciascun fonema (non per conteggio globale)
        duration_per_phoneme = mas_mask.sum(dim=-1, keepdim=True).clamp(
            min=1
        )  # [B, T_text, 1]
        z_pooled = (
            torch.bmm(mas_mask.to(dtype=z_spec.dtype), z_spec.permute(0, 2, 1))
            / duration_per_phoneme
        )

        # x_mask: [B, 1, T_text] dove True = token valido
        # Restituiamo ~x_mask (True = padding) per compatibilità con
        # la convenzione PyTorch src_key_padding_mask
