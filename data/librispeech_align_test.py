...
class LibriSpeechAlignTestDataset(SimpleAudioDataset):
    ...
    def __init__(
        ...
        parquet_dir = os.path.join(slurm_tmpdir, "datasets/librispeech-aligned")

        # Load the dataset from parquet
        local_path = os.path.join(parquet_dir, "test_clean")
        if os.path.exists(local_path):
            print(f"Loading local parquet files from: {local_path}")
            dataset = load_dataset(
                "parquet",
                data_files={"test": f"{local_path}/*.parquet"},
            )
        else:
            raise ValueError(f"Local path {local_path} not found.")
        ...
        for destination in partitions_per_destination:
            ...
            if destination == "test":
                # Always ensure we only use the correct clean subset (test-clean or dev-clean)
                target_subset = (
                    "dev-clean" if "dev_clean" in local_path else "test-clean"
                )
                original_len = len(combined)
                combined = combined.filter(
                    lambda subset: subset == target_subset,
                    input_columns=["subset"],
                    num_proc=min(os.cpu_count(), 8),
                    desc=f"Filtering {target_subset} subset",
                )
                print(
                    f"Kept only {target_subset}: {original_len} -> {len(combined)} samples"
                )
