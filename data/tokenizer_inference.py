...
def main():
    ...
    parser.add_argument("-t", "--tokenizer", type=str, default="bpe.json")
    ...
    for batch in tqdm(dataloader):
        ...
        # Convert Tensor to List[List[int]] for the tokenizer
        if isinstance(audio_codes, torch.Tensor):
            sequences = audio_codes.tolist()
        else:
            sequences = list(audio_codes)

        # Encode
        batch_tokens, batch_durations = tokenizer.encode_batch(sequences)
        encoded_results.extend(batch_tokens)
        durations_results.extend(batch_durations)
        for i in range(len(batch_tokens)):
            compression_ratios.append(len(sequences[i])/len(batch_tokens[i]))
