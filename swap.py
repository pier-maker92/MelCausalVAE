...
def main(args):
    ...
    with torch.inference_mode():
        ...
        # Encode both
        feat_in, mask_in, _, _, _ = model.extract_features([(wav_in, model.config.sample_rate)])
        feat_tg, mask_tg, _, _, _ = model.extract_features([(wav_tg, model.config.sample_rate)])
        ...
        if speaker_embedding_tg is None:
            print("Warning: target speaker embedding is None. Ensure the model was trained with use_instance_norm=True.")
